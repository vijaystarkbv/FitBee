...
print("=" * 60)
print("EXPORTING FITBEE NECK EXTENSION GLB WITH BRAND NEW ACTION")
print("=" * 60)

anim_dir = os.path.join(os.getcwd(), "public", "assets", "models", "animations", "neck")
os.makedirs(anim_dir, exist_ok=True)
anim_glb_path = os.path.join(anim_dir, "neck_extension.glb")
...
if armature_obj:
    ...
    neck_ext_action = bpy.data.actions.new(name="FitBee_Neck_Extension_Action")
    armature_obj.animation_data.action = neck_ext_action
    ...
    ext_neck_angle = -math.radians(32)
    ext_head_angle = -math.radians(14)
    ...
    # Frame 55 to 75 (0.67s): Peak Backward Extension Hold
    if neck_bone:
        neck_bone.rotation_euler = (ext_neck_angle, 0, 0)
        neck_bone.keyframe_insert(data_path="rotation_euler", frame=55)
        neck_bone.keyframe_insert(data_path="rotation_euler", frame=75)
    if head_bone:
        head_bone.rotation_euler = (ext_head_angle, 0, 0)
    ...
    print(f"✅ Verified Action '{neck_ext_action.name}' active on Armature.")
...
print(f"✅ Successfully exported isolated Neck Extension GLB: {anim_glb_path} ({os.path.getsize(anim_glb_path)} bytes)")
print("=" * 60)
